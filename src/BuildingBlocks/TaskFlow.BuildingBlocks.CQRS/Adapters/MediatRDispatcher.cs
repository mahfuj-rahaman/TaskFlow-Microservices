...
using MediatR;
using TaskFlow.BuildingBlocks.CQRS.Abstractions;

namespace TaskFlow.BuildingBlocks.CQRS.Adapters;
...
public sealed class MediatRDispatcher : IDispatcher
{
    ...
    {
        return _mediator.Send((IRequest<TResponse>)command, cancellationToken);
    }

    public Task<TResponse> QueryAsync<TQuery, TResponse>(TQuery query, CancellationToken cancellationToken = default)
        where TQuery : IQuery<TResponse>
    {
        return _mediator.Send((IRequest<TResponse>)query, cancellationToken);
    }
